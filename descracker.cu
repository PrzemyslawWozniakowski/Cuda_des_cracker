...
__global__ void Crack_Kernel(uint64_t data, uint64_t encodedData, uint64_t *crackedkey, bool *foundFlag, uint64_t minKeyVal, uint64_t maxKeyVal, int keyLength);
...
int main()
{
	...
	int numberOfBlocks = 1024, numberOfThreadsPerBlock = 32;
	dim3 grid(numberOfBlocks, 1, 1);
	dim3 threads(numberOfThreadsPerBlock, 1, 1);

	cudaCheckErrors(cudaMallocManaged((void**)&deviceKey, sizeof(uint64_t)));
	cudaCheckErrors(cudaMallocManaged((void**)&wasCracked, sizeof(int)));
	...
	uint64_t keyRange = maxKeyVal / deviceCount;
	if (maxKeyVal%deviceCount != 0) keyRange++;
	std::chrono::system_clock::time_point start = std::chrono::system_clock::now();
	for (int i=0 ; i < deviceCount; i++)
	{
		cudaCheckErrors(cudaSetDevice(i));
		Crack_Kernel << <grid, threads >> > (dataToEncrypt, encryptedMessage, deviceKey, wasCracked, (i)* keyRange,(i+1)* keyRange, keyLength);
	...
	uint64_t encryptedDataWithKeyFromCPU = EncryptData(dataToEncrypt, crackedKeyCPU);
	if (encryptedDataWithKeyFromCPU==encryptedMessage)
	{
...
__global__ void Crack_Kernel(uint64_t data, uint64_t encodedData, uint64_t *crackedkey, bool *foundFlag, uint64_t minKeyVal, uint64_t maxKeyVal, int keyLength)
{
	for (uint64_t i = blockIdx.x * blockDim.x + threadIdx.x + minKeyVal; i <= maxKeyVal; i += blockDim.x * gridDim.x)
	{
